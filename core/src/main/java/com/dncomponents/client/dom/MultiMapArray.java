...
public class MultiMapArray<K, V> {
    private final Map<K, List<V>> multimap = new HashMap<>();
    ...
    public int size() {
        return this.multimap.values().stream().mapToInt(List::size).sum();
    }

    public Set<V> values() {
        return (Set<V>) multimap.values().stream().flatMap(List::stream);
    }
